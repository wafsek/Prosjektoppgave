...
/**
 * The class that handles input validation, registration of data, and in
 * general all communication between front end and back end.
 * 
 * @author Baljit Sarai
...
public class Controller {
    ...
    public void start() {
        ...
        specificClaimAdviceScene = new SpecificClaimAdviceScene(
                guiEventHandler, keyPressHandler);
    ...
    private void launchSpecificInsuranceScene() {
        ...
        if (insurance instanceof HomeInsurance) {
            primaryStage.initiate(
                    specificInsuranceScene.getHouseInsuranceInfoScene(
                            (HomeInsurance) insurance));
        } else if (insurance instanceof TravelInsurance) {
            primaryStage.initiate(
                    specificInsuranceScene.getTravelInsuranceInfoScene(
                            (TravelInsurance) insurance));
        } else if (insurance instanceof BoatInsurance) {
            primaryStage.initiate(
                    specificInsuranceScene.getBoatInsuranceInfoScene(
                            (BoatInsurance) insurance));
        } else if (insurance instanceof CarInsurance) {
            primaryStage.initiate(
                    specificInsuranceScene.getCarInsuranceInfoScene(
    ...
    public void handleControl(Control control) {
        if (control == loginScene.getLoginButton()) {
            ...
            CurrentStatus.setCurrentInsurance(
                    new UpdateInfoWindow().updateInsurance(
    ...
    private String registerPerson(){
        if(!this.validatePersonData().equals(DataControl.SUCCESS.getDescription())){
            ...
            Customer customer = new Customer(
                    addScene.getBirthNumberFieldText(),
                    addScene.getFirstNameFieldText(),
                    addScene.getLastNameFieldText(),
                    addScene.getTelephoneNumberFieldText(),
                    addScene.getEmailFieldText(),
                    addScene.getZipCodeFieldText(),
                    addScene.getAdressFieldText(),
                    addScene.getBillingZipCodeFieldText(),
                    addScene.getBillingAdressFieldText());
    ...
    public String registerHomeInsurance() {
        if (houseInsuranceScene.isHolidayHome()) {
            if (!this.validateHomeInsuranceData().equals("Success")) {
                ...
                return "Boligforsikring registrert";
            }
        } else {
    ...
    public String registerHolidayHomeInsurance() {
        if (!this.validateHomeInsuranceData().equals("Success")) {
            ...
            manager.registerHolidayHomeInsurance(new HolidayHomeInsurance(
                    ...
                    CurrentStatus.getCurrentCustomer());
            DataBank.saveDataBank();
            return "Ferieboligforsikring registrert";
        }
    }
